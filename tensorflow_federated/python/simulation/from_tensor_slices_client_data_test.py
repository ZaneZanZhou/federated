...
TEST_DATA_WHERE_DATA_FOR_CLIENTS_ARE_ORDEREDDICTS = {
    'CLIENT A':
...
TEST_DATA_WHERE_DATA_FOR_CLIENTS_IS_PART_LIST_AND_PART_DICT = {
    'CLIENT A':
        collections.OrderedDict(
            x=np.asarray([[1, 2], [3, 4], [5, 6]], dtype='i4'),
            y=np.asarray([4.0, 5.0, 6.0], dtype='f4'),
...
class FromTensorSlicesClientDataTest(tf.test.TestCase):
  ...
  def test_where_client_data_is_ordered_dicts(self):
    ...
    self.assertCountEqual(
        TEST_DATA_WHERE_DATA_FOR_CLIENTS_ARE_ORDEREDDICTS.keys(),
    ...
    for client_id in TEST_DATA_WHERE_DATA_FOR_CLIENTS_ARE_ORDEREDDICTS:
      self.assertSameDatasetsOfDicts(
          tf.data.Dataset.from_tensor_slices(
              TEST_DATA_WHERE_DATA_FOR_CLIENTS_ARE_ORDEREDDICTS[client_id]),
