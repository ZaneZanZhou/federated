...
def clipping_factory(
  ...
  """Wraps an aggregation factory to perform L2 clipping.
  ...
  The returned `AggregationFactory` will be a `WeightedAggregationFactory` if
  and only if `inner_agg_factory` is weighted.
...
def zeroing_factory(zeroing_norm: Union[float,
  ...
  """Wraps an aggregation factory to perform zeroing.
...
def _make_wrapper(clipping_norm: Union[float,
  ...
  """Constructs `AggregationFactory` that applies clip_fn before aggregation."""
  ...
  def make_init_fn(inner_agg_process):
  ...
  if isinstance(inner_agg_factory, factory.WeightedAggregationFactory):
    ...
    class ClippingFactory(factory.WeightedAggregationFactory):
      ...
      def create(
        ...
        init_fn = make_init_fn(inner_agg_process)
